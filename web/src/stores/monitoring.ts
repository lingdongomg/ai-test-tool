...
export const useMonitoringStore = defineStore('monitoring', {
  ...
  getters: {
    ...
    unresolvedAlerts(): number {
      return this.statistics?.alerts?.unresolved ?? 0
    },
  ...
  actions: {
    ...
    async fetchHealthChecks() {
      ...
      try {
        ...
        const result = await monitoringApi.listHealthCheckExecutions(params) as {
    ...
    async fetchHealthCheck(executionId: string) {
      try {
        this.currentHealthCheck = await monitoringApi.getHealthCheckExecution(executionId) as HealthCheckResult
      } catch {
        this.currentHealthCheck = null
      }
    },

    async runHealthCheck(baseUrl: string, options?: { request_ids?: string[]; tag_filter?: string }) {
      const result = await monitoringApi.runHealthCheck({
        base_url: baseUrl,
        ...options
      })
    ...
    async fetchAlerts() {
      ...
      try {
        const params = {
          ...
          is_resolved: this.alertsFilters.isResolved ?? undefined
        }
