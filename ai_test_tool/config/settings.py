...
class LLMConfig(BaseModel):
    ...
    model: str = Field(default="lfm2.5-thinking:latest", description="模型名称")
    api_key: str | None = Field(
        default=None, description="API密钥（对于需要认证的服务）"
    )
    ...
    max_tokens: int = Field(
        default=8192, description="最大生成token数（建议8192以上，避免输出截断）"
    )
    debug: bool = Field(default=False, description="是否开启LangChain调试模式")
    ...
    def from_env(cls) -> Self:
        ...
        return cls(
            ...
            model=os.getenv("LLM_MODEL", "lfm2.5-thinking:latest"),
...
class LogParserConfig(BaseModel):
    ...
    chunk_size: int = Field(default=500, description="每次处理的日志行数")
    max_lines: int | None = Field(default=None, description="最大处理行数")
    supported_formats: list[str] = Field(
        default=["json", "jsonl", "text"], description="支持的日志格式"
    )
...
class TestConfig(BaseModel):
    ...
    base_url: str = Field(
        default="http://localhost:8080", description="测试目标基础URL"
    )
...
class SecurityConfig(BaseModel):
    ...
    debug: bool = Field(default=False, description="调试模式（生产环境应关闭）")
    ...
    upload_max_size: int = Field(
        default=10 * 1024 * 1024, description="最大上传文件大小（字节）"  # 10MB
    )
    ...
    def from_env(cls) -> Self:
        ...
        return cls(
            ...
            cors_allow_credentials=os.getenv("CORS_CREDENTIALS", "true").lower()
            in ("true", "1", "yes"),
...
class TaskConfig(BaseModel):
    ...
    max_workers: int = Field(default=4, description="后台任务最大工作线程数")
